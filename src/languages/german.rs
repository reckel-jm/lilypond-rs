//! A module for German LilyPond note input.
...
pub static LANGUAGE_STR: &str = "german";
...
pub fn lilypond_from_note(note: &Note) -> String {
    // We cover the special case in German that the English b flat is covered by a simple B.
    if (note.pitch.note_name == NoteName::B) && (note.pitch.accidental == Accidental::Flat) {
        return format!(
            "b{}{}{}",
            note.pitch.octave,
            note.rhythm.length,
...
fn accidental_from_lilypond(note: &LilyPondNote) -> Result<Accidental, String> {
    ...
    // The German special case German b = English b flat
    if note.get_capture("note_name").as_str().eq("b") {
        return Ok(Accidental::Flat);
    };
